...
// import { LightbulbFilledIcon } from "./icons";
...
const SmartLightCard = () => {
  ...
  const getCurrentSwitchState = useCallback(() => {
    const message = JSON.stringify({
      id: incrimentalId,
      type: "get_states",
    });
  ...
  }, [manualToggle, getCurrentSwitchState]);
    ...
    newSocket.addEventListener("open", () => {
      ...
      newSocket.send(
        JSON.stringify({
          type: "auth",
          access_token:
            "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiI3ZTA5NDg5M2E3RGJhMiIsImlhdCI6MTYyNzA2MTExMywiZXhwIjoxOTQyMzIwMTIzfQ.MzYzNWE3MjA4N2Q4MjhhYTUxNGIyMzk5M2I2NWIxYzk", // Replace with your access token
        })
      );
      newSocket.send(
        JSON.stringify({
